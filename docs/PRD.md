...
* **資源市場初始化**：長條型陣列堆疊，水力（極充沛，自 $1 開填）、火力（中等，自 $3 開填）、綠能（稀少，自 $7 開填）、算力（極稀少，僅在 $14、$16 價位各放 1 個）。
* **卡牌牌庫協議**：初始市場固定抽取編號 `03` 至 `10` 共 8 張。將 `13` 號（純綠能廠）置於牌庫頂，`Step 3` 卡固定置於牌庫底，其餘隨機洗勻。
