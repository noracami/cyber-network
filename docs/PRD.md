...
> 美術第一輪見 [PRD-v1.3.md](PRD-v1.3.md)（圖示系統、地圖換皮、盤面動畫），
> v1.4 見 [PRD-v1.4.md](PRD-v1.4.md)（NPC 與單人遊玩、結束權力下放），
> v1.5 見 [PRD-v1.5.md](PRD-v1.5.md)（牌局持久化、多房間、行動版、介面精修）。
...
* **Admin 掀桌鍵**：【要】提供全域 `ABORT_GAME` API，Admin 可隨時強制清空進行中的遊戲盤面，退回 Lobby 狀態。（v1.4 修訂：**本局入座玩家**亦可結束遊戲——單人配 NPC 遊玩時得以自行收局。）
